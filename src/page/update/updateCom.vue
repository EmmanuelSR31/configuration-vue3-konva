...
<n-modal :title="title" v-model:show="showModel" preset="card" style="width: 550px;" :mask-closable="false" :close-on-esc="false">
  <div class="modal-form">
    <n-form ref="formValidate" :model="dataObj" :rules="ruleValidate" label-placement="left" label-width="80px" require-mark-placement="left">
      ...
      <n-form-item label="条目内容" path="content">
        <n-input v-model:value="dataObj.content" type="textarea" :rows="4" placeholder="请输入条目内容" :disabled="method === 'view'"></n-input>
      </n-form-item>
    </n-form>
    <div class="modal-btn">
...
export default {
  ...
  setup (props) {
    ...
    let dataObj = ref({ content: '', updateLogId: '' }) // 数据对象
    const ruleValidate = reactive({ // 表单验证
      content: [
        { required: true, message: '请填写条目内容', trigger: 'blur' }
      ]
    })
    /**
    * @desc 初始化
    */
    function init () {
      ...
      if (props.method === 'add') {
        dataObj.value.updateLogId = props.leftObj!.updateLogId
      }
    }
    init()
    /**
    ...
    function save () {
      formValidate.value?.validate((errors: any) => {
        if (!errors) {
          ...
          if (props.method === 'add') {
            proxy.$api.post('commonRoot', '/module/updatelog/item/web/insert', dataObj.value, (r: IInterfaceData) => {
              if (r.data.code === 0) {
            ...
            proxy.$api.post('commonRoot', '/module/updatelog/item/web/update', dataObj.value, (r: IInterfaceData) => {
              if (r.data.code === 0) {
                proxy.$myMessage.success('保存成功')
                parentChangePage(1, 10)
                proxy.onClose()
